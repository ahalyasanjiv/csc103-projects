...
namespace cppfsm {
	...
	const set<char> ident_st(s_idst.c_str(),s_idst.c_str()+s_idst.length()); //could be start of an identifier (underscore + all letters)
	const set<char> ident(s_id.c_str(),s_id.c_str()+s_id.length());
	//same as above + numbers
	const set<char> escseq(s_escseq.c_str(),s_escseq.c_str()+s_escseq.length());
		//list of characters that are valid escape sequences like n in \n
	const set<char> iddelim(s_iddelim.c_str(),s_iddelim.c_str()+s_iddelim.length());
	// can delimit identifiers (don't start w/ constant of //) like whitespace
